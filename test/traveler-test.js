...
describe("Traveler", () => {
  ...
  it("should filter through all trips and return the ones whose userID matches travelers ID", () => {
    ...
    expect(traveler.filterTripData(tripsData)).to.deep.eq([tripsData[3], tripsData[4], tripsData[5]]);
    expect(traveler2.filterTripData(tripsData)).to.deep.eq([tripsData[0], tripsData[1], tripsData[2]]);
  });
